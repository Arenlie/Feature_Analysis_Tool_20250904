...
def dataTo2700table(inputFile, outputFile):
    ...
    all_cards_2 = [f"C{str(i).zfill(2)}" for i in range(1, 3)]  # C01-C04
    all_channels_1 = [f"CH0{j}" for j in range(1, 5)]  # CH01-CH04
    all_channels_2 = [f"CH0{j}" for j in range(1, 9)]  # CH01-CH04
    ...
    for index, row in data_all.iterrows():
        if str(row["通道编码"])[:6] == "50294D":
            ...
            row_data = {
                ...
                "通道编号": channel_number, "测点（通道）类型": "动态电压" if row["传感器类型"] in ["电流谱", "电压谱"] else row["传感器类型"],
                ...
                "板卡类型": "高速卡" if row["传感器类型"] in ["加速度", "速度", "位移", "转速", "应力波", "电流谱", "电压谱"] else "低速卡",
...
if __name__ == "__main__":
    dataTo2700table(r"D:\项目资料\特征解析工具汇编\PHM2.0打包\data_all - 2300测试.xlsx",
